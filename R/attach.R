core <- c("saros.base", "saros.contents", "saros.utils")
...
# loaded from before. https://github.com/saros/saros/issues/171
same_library <- function(pkg) { #
  loc <- if (pkg %in% loadedNamespaces()) dirname(getNamespaceInfo(pkg, "path"))
  library(pkg, lib.loc = loc, character.only = TRUE, warn.conflicts = FALSE)
}

saros_attach <- function() { #
...
saros_attach_message <- function(to_load) { #
  if (length(to_load) == 0) {
    return()
  }

  header <- cli::rule(
    left = cli::style_bold("Attaching core saros packages"),
    right = paste0("saros ", package_version_h("saros"))
  )
