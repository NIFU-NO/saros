...
get_docx_dims <- function(docx_file) {
  ...
  docx_dims <- c(
    w = docx_dims$page[["width"]] -
      docx_dims$margins[["left"]] -
      docx_dims$margins[["right"]],
    h = docx_dims$page[["height"]] -
...
get_docx_height <-
  function(
    ...
    min(c(
      plot_height_fixed_constant + plot_height_multiplier * n_col,
      minimum_height
    ))
  }


check_autonum <- function(
  x,
  ...
  if (!is.null(x) && !inherits(x, "run_autonum")) {
    cli::cli_abort(
      stringi::stri_c(
        ignore_null = TRUE,
        "If not NULL, ",
        err_msg("n object from {.fun officer::run_autonum}")
      ),
      call = call
    )
  }
