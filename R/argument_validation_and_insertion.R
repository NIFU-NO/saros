argument_validation_and_insertion <- function(params) {
  ...
  arg_params <-
    list(
      ...
      qmd_start_section_filepath = list(fun = function(x) is.null(x) || (is_string(x) && file.exists(x))),
      qmd_end_section_filepath = list(fun = function(x) is.null(x) || (is_string(x) && file.exists(x))),
