...
check_colour_palette <- function(colour_palette, call = rlang::caller_env()) {
  if (
    !is.null(colour_palette) &&
      (!all(is_colour(colour_palette)) ||
...
get_remaining_colours <- function(
  user_colour_set,
  n_colours_needed,
  ordinal = FALSE
) {
  ...
  if (!is.null(user_colour_set)) {
    ...
    if (length(user_colour_set) >= n_colours_needed) {
      if (ordinal) {
        ...
        return(user_colour_set[seq_len(n_colours_needed)])
      }
    } else {
      cli::cli_warn(
...
get_colour_set <-
  function(
    ...
    if (
      ...
      tryCatch(
        x[!x %in% colour_na] <- colours_available[seq_along(x[
          !x %in% colour_na
        ])], ## PERHAPS NOT PERFECT HERE?
        warning = function(e) {
          cli::cli_warn(stringi::stri_c(
            ignore_null = TRUE,
            "colours_available: {colours_available}. 'x' yields {x}.",
            e
          ))
        },
        error = function(e) {
...
get_colour_palette <-
  function(
    ...
    if (length(out) > 1) {
      cli::cli_warn(
        "Multiple colour palettes embedded in {col_pos}. Using first one."
      )
    }
    if (length(out) >= 1) {
      out <- out[[1]]
    ...
    common_data_type <-
      get_common_data_type(data = data, col_pos = col_pos)

    ## If nothing specified, use the default palette
    common_levels <-
      get_common_levels(data = data, col_pos = col_pos)
    ...
    tryCatch(
      out[!out %in% colour_na] <- colours_available[seq_along(out[
        !out %in% colour_na
      ])], ## PERHAPS NOT PERFECT HERE?
      warning = function(e) {
        cli::cli_warn(stringi::stri_c(
          ignore_null = TRUE,
          "colours_available: {colours_available}. 'out' yields {out}.",
          e
        ))
      },
      error = function(e) {
