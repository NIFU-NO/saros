...
check_colour_palette <- function(colour_palette) {
...
#' @param names Optional names of colour vector, follows same options as rlang::set_names.
  ...
  function(n_colours_needed,
    ...
    if(!is.null(user_colour_set)) {
      ...
      if(length(user_colour_set) >= n_colours_needed) {
        ...
        cli::cli_abort("Fewer colours provided in colour palette than needed.",
                      call = call)
      }
    ...
    out <- rlang::set_names(out, nm = names)
