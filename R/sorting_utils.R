...
#' `add_indep_order()` has already resolved `sort_indep_by` and
#' `descend_indep` into it. It is only present for categorical dependent
#' variables; `summarize_int_cat_data()` does no sorting, so a numeric
#' dependent variable falls back to the factor's own levels -- which is what
#' `arrange_table_data()` does in the same situation.
#'
#' @param data_summary Summarized data, possibly carrying `.indep_order`
#' @param data Raw dataset, used for the factor-level fallback
#' @param indep Name of the independent variable, or `NULL`
#' @return Character vector of category labels in order, or `NULL`
#'
#' @keywords internal
get_indep_level_order <- function(data_summary, data, indep) {
  if (!rlang::is_string(indep)) {
    return(NULL)
  }
  ...
  if (
    ...
    levels_out <- unique(as.character(ordered[[indep]]))
    return(levels_out[!is.na(levels_out)])
  }

  if (!indep %in% names(data)) {
    return(NULL)
  }
  values <- data[[indep]]
