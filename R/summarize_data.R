...
add_collapsed_categories <- function(data_summary, sort_by = NULL, call = rlang::caller_env()) {
...
sort_data <- function(data_summary, sort_by = NULL, desc = FALSE,
                      call = rlang::caller_env()) {
  ...
  if(is.null(sort_by)) {
    ...
    #### DESC IS CURRENTLY GLOBAL ACROSS ALL VARIABLES:
...
#' @param percentage [\code{logical(1)}]\cr Whether to compute percentage or frequency (default).
