...
prep_checkbox_prop_plot_html <-
  function(data,
    ...
    p <-
      ...
      dplyr::mutate(
        .id = seq_len(nrow(.)),
        .category = factor(.data[[".category"]], levels = rev(levels(.data[[".category"]]))),
        .data_label = ifelse(as.character(.data[[".category"]]) == names(colour_palette)[1],
                             NA, .data[[".data_label"]]),
        .variable_label = forcats::fct_reorder(.data[[".variable_label"]],
                                               .data[[".mean"]]),
...
#' @examples
#' \dontrun{
#' embed_checkbox_prop_plot(data = ex_survey1, dep = b_1:b_3)
#' }
