...
## New Features
-   `crowd_plots_as_tabset()` and `crowd_tables_as_tabset()` now automatically insert `\newpage` page breaks between items when rendering to non-HTML formats (docx, typst, PDF). This prevents figures and tables from being cut off due to insufficient page space when tabsets are rendered sequentially in paginated output formats
