...
#' \item{".label"}{Sort by the variable labels.}
...
#' @examples
#' # library(tidyselect)
#' # surveyreport:::prepare_mschart_data(ex_survey1, cols = matches("^b_"),
  ...
  function(data,
    ...
    if(ncol(dplyr::select(.data = data, {{by}})) == 1L &&
      ...
      data <-
        data %>%
        dplyr::select({{cols}}) %>%
        crosstable::crosstable(percent_pattern = "{n}",
                               showNA = showNA,
                               label = TRUE)
    ...
    if(grepl("per", what)) {
      data <-
        ...
        dplyr::mutate(data_label = .data$value/sum(.data$value, na.rm=TRUE)*100,
    ...
    data <-
      data %>%
      dplyr::group_by(.data$label) %>%
      dplyr::mutate(n_unique = dplyr::n_distinct(.data$variable, na.rm=FALSE)) %>%
      dplyr::ungroup()
    ...
    if(!is.null(sort_by)) {
      ...
      data <-
        data %>%
        dplyr::group_by(.data$label, .data$cat_id) %>%
        dplyr::mutate(sum_value =
                              dplyr::if_else(.data$cat_id,
                                             sum(as.numeric(.data$value), na.rm=TRUE),
                                             NA_real_)) %>%
        dplyr::ungroup() %>%
        dplyr::arrange(dplyr::desc(.data$cat_id),
    ...
    data %>%
      dplyr::mutate(label = factor(x = .data$label,
                                   levels = .env$lvls,
                                   labels = .env$lbls,
                                   ordered = TRUE),
                    data_label =
                      dplyr::if_else(as.numeric(.data$value) <
                                       as.numeric(.env$ignore_if_below),
                                     "",
                                     as.character(.data$data_label)),

                    mean = as.integer(.data$variable)*.data$value) %>%
      dplyr::arrange(as.integer(.data$label), .data$variable)
  }
...
#'
#' @examples
#' #create_chart_likert(prepare_mschart_data(ex_survey1[paste0("b_", 1:3)]))
create_chart_likert <-
  function(data,
    ...
    checkmate::assert_subset(x = colnames(data), choices = c(y,x,group,labels, ".id", "cat_id", "sum_value", "mean"), add = coll)
...
#' @param ignore_if_below [\code{numeric(1)}]\cr Whether to hide label if below this value.
...
report_chart_likert <-
  function(data,
    ...
    if(!is.null(label_separator) &&
      ...
      docx_file <- officer::body_add_caption(x = docx_file, value = block_caption)
    }
    ...
    docx_file <- mschart::body_add_chart(x = docx_file,
                            chart = chart,
