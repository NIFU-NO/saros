...
#' Prepare data for significance test
#'
#' @param data Data frame or tibble.
#' @param response Response variable(s), as bare names (tidyselect
#' @param explanatory Explanatory variable(s), as bare names (tidyselect)
#'
#' @return Dataframe with specified variables modified. Same row number.
#'
#' @examples .prepare_sigtestdata_ordinal(data = ex_survey1,
#'                                        cols = matches("b_"))
.prepare_sigtestdata_ordinal <-
  function(data, response = NULL, explanatory = NULL) {
    dplyr::mutate(data,
...
#' Extention to the infer package for detecting and dispatching correct test
#'
...
#' @param check If TRUE (default), checks arguments and data. Set to FALSE to speed up.
#'
...
.create_sigtest <-
  function(x,
    ...
    if(check) {
      ...
      if(length(tidyselect::eval_select(rlang::enquo(response), x)) > 1L) {
        cli::cli_abort("This function only handles a single response column. Please use test_multiple_comb() for multiple test combinations.")
      }



      x <- .prepare_sigtestdata_ordinal(x,
                                  response = {{response}},
      ...
      purrr::map_dfr(.x = expl_cols,
                     ...
                     .f = function(explanatory) {
                       ...
                       if(test == "t") {
                         ...
                         miss <- dplyr::filter(x, !is.na({{response}}), !is.na(!!expl_col))
                         if(nrow(miss)>3 &&
...
#' Significance testing of multiple variable combinations with easy test dispatching.
#'
#' Extention to the {infer} package
#'
#' @inheritParams .create_sigtest
#'
#' @importFrom dplyr mutate across bind_cols summarize group_by n
#' @importFrom tidyselect eval_select
#' @importFrom labelled remove_val_labels
    ...
    out <-
      purrr::map_dfr(.x = resp_cols,
                   ...
                   .f = function(response) {
                     ...
                     .create_sigtest(
                       x = x,
                       response = response, # Sannsynligvis feil
                       explanatory = explanatory, # Sannsynligvis feil
                       order = order,
                       alternative = alternative,
                       mu = mu,
                       correct = correct,
