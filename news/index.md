...
- `sigtest_table_html` no longer aborts with
  ...
  variable once it had more than five distinct values, and
  [`unique()`](https://rdrr.io/r/base/unique.html) counted `NA` as one
  of them, so an `indep` with exactly five observed categories plus any
  missing value tripped that limit. The drop happened inside a loop and
  was invisible to the wide pivot that then asked for the column by
  name, which is why the abort was identical for every `showNA` setting.
  The limit is gone (see below), so the grouping survives however many
  missing values `indep` has. `showNA` still decides whether the `NA`
  group gets columns of its own
  ([\#603](https://github.com/NIFU-NO/saros/issues/603))
- The undocumented five-category limit on the independent variable has
  been removed, so `sigtest_table_html`, `int_table_html` and
  `int_plot_html` now honour an `indep` with any number of categories.
  Previously a six-category `indep` broke all three even with no `NA`
  present: the table types aborted and the plot failed at draw time,
  because the descriptives were silently computed without the grouping
  ([\#603](https://github.com/NIFU-NO/saros/issues/603))
- `sigtest_table_*` now respects `n_categories_limit` (default 12),
  falling back to a long-format table when the independent variable has
  more categories than that, instead of adding a set of columns per
  category without limit. `make_content.sigtest_table_html()` also
  renames its `Var 1`/`Var 2` columns before attaching the descriptives,
  so variables actually named `x` or `y` are no longer mangled into
  `x...2`/`x...6` ([\#603](https://github.com/NIFU-NO/saros/issues/603))
...
- Fixed `hide_axis_text_if_single_variable` parameter not being
  respected in `cat_plot_docx` output. The function now properly hides
  axis labels for single-variable plots when this parameter is `TRUE`,
  matching the behavior of `cat_plot_html`
