...
- **Major change:**
  ...
  served `NULL`)
- **Major change:** Resolved issue
  [\#372](https://github.com/NIFU-NO/saros/issues/372) - `descend`
  parameter now works correctly with ordered factors while preserving
  their inherent level ordering. Ordered factors maintain their natural
  order as the base, but `descend` can reverse the display order
...
- [`n_rng2()`](https://nifu-no.github.io/saros/reference/n_rng2.md) now
  ...
  calculating N from complete cases instead of aborting
- Added handling for all-NA and empty `.count_per_indep_group` values,
  returning “0” with clear warnings instead of producing infinite values
...
- Added comprehensive unit tests for
  [`makeme()`](https://nifu-no.github.io/saros/reference/makeme.md)
  variable type checking edge cases including all-NA variables
