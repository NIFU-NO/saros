...
- caption: translations are not included. 


- implement uni_plot_int_html and bi_plot_int_html 
- separate settings for each element type in  (and thus also in mass_lst_element)
- sigtest-related
    - show_if_p_below
    - replace_sigtest_with_n_if_n_below
...
- nothing is actually saved in raw_elements...
...
- a more delicate treatment of options()$saros which create an entry if it does not already exist. Loop across all .saros.env defaults.
- download figure data as xlsx (priority in KU)
    - toggle between figure and table
- org-spesifikke nettstedspakker
- sub-groups (resp in Spørringene, etc.): Must do like Karl did and create interaction indeps.
- all yaml in the index.qmd into yaml_report, and call it?
- replace all absolute paths with relative paths in get_element_path, etc
- YAML: space before and after chunks?
...
-	plot_cat_docx: Colours are not correct if first category in the first variable is completely missing, at least in Tett på realfag, "I hvilken grad benytter du deg av følgende undervisningsformer i naturfagundervisningen"
    forcats::fct_unify() in prep_plot?
