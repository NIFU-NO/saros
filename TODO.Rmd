...
- plots: 
    - hiding facet now happens with n_cols>1! (for PREP and FREQ)
    ...
    - interactivity...
    - when there is no colour_2nd_binary_cat, 
    update docx and pdf to match html
- tables: 
    - total row in bi-tables?

- chapter initial r chunk with options and useful packages
    - ggiraph set global options within qmd/yaml
    - tidyverse
    - 
- colours
...
- warn if label_separator is found more than once in a label
- N in legend for each by group
    - add_n_to_bygroup => yaml
- implement t-test etc in early removal of bivariates
- implement uni_plot_int_html and bi_plot_int_html 
...
- prep_cat_prop_plot_pdf: Implement n_right_side for when tooltips are unavailable
- download figure data as xlsx (priority in KU)
    - toggle between figure and table using ::: {.panel-tabset} + heading for each tabset
- sub-groups, interaction terms or "stratification" (resp in Spørringene, etc.)
- simpler dep_cat notation: allow spaces, not having to use matchesP?
- reintroduce dep_txt
- only call summarize_data once and reuse.
- save settings with the object(s)/log?
...
- Might be hard to store in memory/Rds the docx-stuff. Perhaps better to create separate docx, then stitch together?
    - https://github.com/tidyverse/rvest/issues/181
    - body_add_docx

# Version 2.0
- Add full _quarto.yml file for project.
    - Should each report be its own website?
- Always hide heading if nothing to show within that level. If data_overview suggests it exists, it must exist, otherwise empty heading?
- descend is currently global across all variables...
- count number of rows removed with hide_bi_entry_if_sig_above

# Version 3.0
- different colours for bi-plot with two groups: Slightly brighter for second group
