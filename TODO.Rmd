...
# Version 1.0
x   plot_html comes twice instead of one html and one pdf
x   No bivariates at all for s_303 or s_30w (in SSN)
x   s_303w_8_fct_sentind3d_kom_BY_sentind3d_kom
x   obj_name: .variable_name_indep shows as NA for univariates (still)
x   Avoid sorting first on .element_name and then indep
x   Clean up the obj_name and filename_prefix stuff.
x   Løse skript for stolpediagrammer, kart (signifikante forskjeller på fylke/landsdel/kommune), tabeller
x   Redesign entire refine_chapter_overview, remove_non_significant_bivariates and sigtest
x   No bivariates?
x   not keeping significant relationships! Seems all or nothing?
x   Can find_test be combined with find_stat_config?
x   Methodize
x   Inconsistencies in chisq.test() and etc.
x   s_801 and s_804 (Maria) are identical?
x   chapter datasets do not contain data and data format is weird?
x   (facet strips 90 deg)
x   In nifudocx.docx, turn off numbering? or opposite?
x   Add to chapter yaml template: format: nifudocx-docx
x   if(all(is.na(chapter_overview_section$.variable_name_dep)) && 
    rlang::is_string(dots$mesos_group)) => all chapter_overview_section becomes NA
x   figure tooltip should also include x_var

1   Generally, should allow users to adjust as much as possible later. Make fewer absolute changes?
1   Word documents are not being saved as a battery but as just one variable...
1   PDF-figures are way too big
3   Better workflow: No copying of subfolders. INstead of personal work folders, chapter folders. 
        Rapporter/2024H/03_Lykke/index.qmd (Problem in that confusing qmd file name...)
        Rapporter/2024H/03_Lykke/03_lykke/...
2   Test replace_docx_imgs_with_mscharts
2   Better password/username system for institutions: Distinguishing folder name and username?
2   Quarto issue: for typst, setting fig-height in chunk (without also setting fig-dpi) results in "Error in `s$close()`: ! attempt to apply non-function
3   Return path to index-file => path to folder, replace \ with /
...
3   Better checks of input data: Provide feedback/log on included variables, excluded due to non-significance, 
...
3   Call summary_data only once in gen_element_and_qmd_snippet3
2   Move common procedures in the dataprep scripts into saros to avoid accidental issues
...
x   no pval is showing!
x    hide_sigtest_if_n_below seems too strict?
x    Hide entire table if nothing to display?
3   overkill with main_question in label + as label-column header
x   variable	.variable_label	test	stat	p
3   Have some translations-headers?
...
3   Also, a lighter inactive shadow alpha. 
