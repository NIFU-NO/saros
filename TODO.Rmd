...
# Version 1.0
1   If a data.frame is submitted as data in draft_report(), only a single row is used...
1   Must find out why AGU creates a report within the mesos/næring folder...
        Probably because of NA on the mesos_var!
1   Must get remove_empty_headers() to work!
1   Ensure ordinal scales in AGU, DU, etc.
1   #| prefer-html: false doesn't work with typst
  1   With typst, is it really needed to have the pdf-conditionals for ggiraph/ggplot?

1   In CriThiSE, there are no bivariates??? Maybe coincidence
2   Reduce level 2 header font?
1   read_main_password_file
1   create__headers_file
1   Finne ut av hvorfor jeg hadde satt execute_dir = getwd() når det medførte at kapittel.qmd ikke fant filer i undermappene. Antageligvis fordi det er enkelte maler i R-skript som jeg ønsket å lese inn fra Ressurser-mappen. Disse kunne jeg ikke finne på andre måter. Kanskje vurdere å bruke environment variabelen. 

# nifutemplate
2   SSN_methodchapter_inlines.R and SSN_methodchapter.qmd
1   functions to quickly download files to a folder
...
# plots
1   gglikert()
2   Interactivity on a single cell, not the group. 
2   Also, a lighter inactive shadow alpha. 
2   hover contains variable_name 
