...
1   pre-render script: build_report() functio, which takes a Report.qmd with YAML-header containing the typst-stuff and other preface-files (and the Report.qmd file(s) cannot be placed in the input folder... index.qmd contains a link to a typst-based PDF of the entire report ({{< include file.qmd >}}).

1   Argument: drop empty sections before rendering (in the render_site() or post-render lua)
1   Update saros according to last version of SSN
        Resources => nifutheme

3   Sjekk ut hypothes-is muligheter, skru av, moderator, osv.
2   nifutheme: trekke ut alle filer til nifumaler
...
1   methodchapter_inlines.R and methodchapter.qmd
...
# Chapter overview
1   not keeping significant relationships!
2   No var label will cause entire chapter to be NA...
...
# Sigtest
2   waning: n min(table(data[, c(dep_pos, indep_pos)])) :
...
# mesos
2   Argument for hva som er faner: c("element", "mesos_var", "dep") 
3   Early fail if mesos_var contains all NA
...
4   distinguish between qmd_start/end for chapters and indexes?
...
4   Separate subfolders for PNG, DOCX (grraphs), TXT, XLSX
    - could use extension as input to function that saves, which adds folder name and extension

# tables

2   tri_catcatcat_table
-   vertical argument in tables provides a pivot-string similar to SSB: 1,2,3
...
-   have Claes compare everything

# DOCX (NIFU stil/saros.template):

-   nifutheme: tables should be centered by default
-   nifutheme: tables are not having right style?
...
2   RStudio Quarto: Copying table from Word into Visual editor results in empty cols and rows
2   RStudio Quarto: Copying figure from Word into Visual editor results in image not found
...
-   multiple levels of by (z-column in chapter_overview?)
