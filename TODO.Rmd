...
+ Titles are missing from both macro and mesos
+ mesos reports no longer producing mesos groups!
+ header numbers: 
    chapter_number <- match(chapter_name, unique(chapter_overview$chapter))
    stringi::stri_c("number-offset: ", chapter_number, ignore_null=TRUE) 

-   Sharepoint!!!
    - Shorten chunk names
-   distinguish between qmd_start/end for chapters and indexes?
-   Konsekvent fargepalett på tvers av figurer med samme svarskala
-   Quarto Issue: fig-cap + label + no graph =\> error
-   functions to simplify copying of files?

-   saros.template::use_saros() function that

    -   asserts being in a project
    -   if so, copies files from an argument package (nifutheme)
    -   alternatively, a function in nifutheme::use_saros()
    -   dumps these files in a resources folder?

-   Allow empty chapter_overview
...
-   for_editor article
-   for_data_cleaner article
-   for_end_user article
...
-   implement uni_plot_int\_\* bi_plot_intint\_\* bi_plot_intcat\_\*
...
-   sarosverse imports:
    -   saros.docx - saros.html - saros.folders - saros.text - saros.psych - saros.effect - saros.qual those in turn import - saros.base - saros.graphic - saros.utils
